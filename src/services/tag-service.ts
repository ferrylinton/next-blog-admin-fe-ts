...
import { Tag, TagForm } from "@/types/tag-type";
...
export async function createTag(data: TagForm, clientInfo: ClientInfo) {
    const headers = getHeaders(clientInfo);
    return await blogApiClient.post('/api/tags', data, { headers });
};

export async function updateTag(id: string, data: TagForm, clientInfo: ClientInfo) {
    const headers = getHeaders(clientInfo);
